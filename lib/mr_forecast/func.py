...
def split_hyper_linear(hyper):
	...
	'''
	c0, slope,sigma, trans = 	hyper[0], hyper[1:1+n_pop], hyper[1+n_pop:1+2*n_pop], hyper[1+2*n_pop:]
...
def classification( logm, trans ):
	...
	print('Terran %(T).1f %%, Neptunian %(N).1f %%, Jovian %(J).1f %%, Star %(S).1f %%' 			% {'T': prob[0], 'N': prob[1], 'J': prob[2], 'S': prob[3]})
	return None
