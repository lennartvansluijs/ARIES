...
from aries.systemparams import systems, calc_transit_duration
...
use_crosscorr_weighting_scheme = True
TTEST_TRAIL_WIDTH = 3
...
if run_bl19_gridsearch or run_bl19_multinest:
    if template_mode == '1D':
        h = BrogiLineBayesianFramework_1DTemplate(spectralorders, templateorders, planetary_system=systems['wasp33'])
    elif template_mode == '2D':
        h = BrogiLineBayesianFramework_2DTemplate(spectralorders, templateorders, planetary_system=systems['wasp33'])
    else:
...
if run_crosscorr:
    ...
    axes = plot_detection_matrix(snrmatrix, dkp_all=dkp_all, dvsys_all=dvsys_all,
                                 kp=systems[targetname]['kp'], vsys=systems[targetname]['vsys'],
    ...
    suptitle = r'Average C-C Matrix (Expected planet params: vsys={:.2f} km/s, Kp={:.2f} km/s)'.format(systems[targetname]['vsys']/1e3, systems[targetname]['kp']/1e3)
    rvplanet = -vbary_combined + systems[targetname]['vsys'] + systems[targetname]['kp']*np.sin(phase_combined*2*np.pi) # rv trial, shifted to the planet's rest frame
    ccmatrix_combined_shifted = shift_to_new_restframe(ccmatrix_combined, rv0=rv_sample, rvshift=rvplanet)
...
if run_wilks_theorem and os.path.exists(loglike_f):        
    ...
    try:
        axes = plot_detection_matrix(wilksmap, dkp_all=dkp_all, dvsys_all=dvsys_all,kp=systems[targetname]['kp'], vsys=systems[targetname]['vsys'],
